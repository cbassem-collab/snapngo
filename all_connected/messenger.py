...
def get_task_info(task_id):
    '''
    Take user id and task id, return information of that task
    '''
    conn = helper_functions.connectDB(DB_NAME)
    cur = conn.cursor()
    query = f'''SELECT id, location, description, start_time, time_window, compensation
                FROM tasks
                WHERE id = {task_id}'''
    ...
    info = cur.fetchone()
    conn.close()
    assert info, f"Task #{task_id} could not be found in database!"
    return info


def get_available_tasks(db_name):
    '''
    Get all the tasks that have available slots together with each task's details. 
    Create a list of available tasks (with details) 
    Return the list
    '''
    update_tasks_expired()
    conn = helper_functions.connectDB(db_name)
    cur = conn.cursor()
    query = '''SELECT id, location, description, start_time, time_window, compensation
                FROM tasks
                WHERE (expired != 1 AND slot > 0)'''
    cur.execute(query)
    tasks = cur.fetchall()
    conn.close() 
    return tasks

def insert_assignments(task, user):
    """
    Takes a task id and a user id.
...
def update_assign_status(new_status, old_status, task_id, user_id):
    '''
    ...
    cur.execute(f"UPDATE assignments SET `status` = '{new_status}' WHERE task_id={task_id} AND user_id='{user_id}'")
    if new_status == "accepted" and old_status != "accepted":
        cur.execute(f"UPDATE tasks SET slot = slot - 1 WHERE id = {task_id}")
    elif new_status == "rejected" and old_status == "accepted":
        cur.execute(f"UPDATE tasks SET slot = slot + 1 WHERE id = {task_id}")
    conn.commit()
    conn.close
...
def get_nearly_expired_tasks(threshold):
    update_tasks_expired()
    conn = helper_functions.connectDB(DB_NAME)
    cur = conn.cursor()
    query = f'''SELECT id
                ...
                WHERE `expired` = 0
                AND slot > 0
                AND (start_time + INTERVAL time_window MINUTE)<(NOW()+ INTERVAL {threshold} SECOND)'''
    cur.execute(query)
    tasks = cur.fetchall()
    conn.close()
    return tasks
