...
def generate_message(task_info, user_id):
    ...
    '''
    block = []
    print('TASK INFO 3', task_info[3])
    starttime_format = task_info[3].strftime("%A (%m/%d) at %I:%M%p")
    ...
    text = (f"PLACEHOLDER_EMOJI *Task #PLACEHOLDER_TASKID* PLACEHOLDER_EMOJI \n*Description:* PLACEHOLDER_DESCRIPTION. \n*Start Time:* PLACEHOLDER_STARTTIME \n*Window:* PLACEHOLDER_WINDOW minutes \n*Compensation:* PLACEHOLDER_COMPENSATION")\
                ...
                .replace('PLACEHOLDER_DESCRIPTION', str(task_info[2])) \
                .replace('PLACEHOLDER_STARTTIME', str(starttime_format)) \
                .replace('PLACEHOLDER_WINDOW', str(task_info[4])) \
                .replace('PLACEHOLDER_COMPENSATION', str(task_info[5])) 
    ...
    buttons = button_color(task_info[0], user_id) # CHANGE
...
def make_report_block(user_id) -> list:
    ...
    all_active = [messenger.get_task_info(task_id) for task_id in active_ids]
    ...
    if BOT_ID != user_id:
        if 'files' not in payload:
            ...
            # User only sends text without attaching an image
                ...
                pending_ids = messenger.get_pending_tasks(user_id)
                all_pending = [messenger.get_task_info(task_id) for task_id in pending_ids]
...
def action_button_click(body, ack, say):
    ...
    if messenger.check_time_window(task) == "expired":
        say(f''':large_orange_circle: Task {task} has already expired. Please pick another task to finish.''')
        return
    if old_status == 'not available':
        say(f''':large_orange_circle: Task {task} has been claimed. Please pick another task to finish or check again later''')
        return
    ...
    if old_status == 'accepted':
        ...
        if old_status == 'pending':
            messenger.insert_assignments(task, user)
        task_list = messenger.get_task_info(task)
        messenger.update_assign_status(new_status, old_status, task, user)
