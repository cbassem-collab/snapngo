...
CHECK_EXPIRE_CYCLE = task_parameters.CHECK_EXPIRE_CYCLE

START_HOURS = task_parameters.START_HOURS
END_HOURS = task_parameters.END_HOURS

THRESHOLD = task_parameters.EXPIRE_THRESHOLD
...
def check_expired_call():
    users = messenger.get_reliable_users()
    tasks = messenger.get_nearly_expired_tasks(THRESHOLD)
...
def messenger_bot_call():
    ...
    tasks = messenger.get_available_tasks(DB_NAME)
    bot.send_tasks(tasks)
    print('- sent tasks')
...
def start_all_timers():
    ...
    expire_timer = RepeatTimer(check_expired_call,
                                seconds=CHECK_EXPIRE_CYCLE,
                                minutes=0,
                                hours=0)
    ...
    return task_timer, expire_timer, messenger_timer

def cancel_all_timers(task_timer, expire_timer, messenger_timer):
...
def daily_cycle():
    ...
    task_timer, expire_timer, messenger_timer = start_all_timers()
    # Run time
    ...
    # End all cycles
    cancel_all_timers(task_timer, expire_timer, messenger_timer)
