...
TASK_SLOT = task_parameters.TASK_SLOT # in number of people
...
def random_datetime(n):
    ...
    * Helper function for create_task() *
    ...
    # Get end time
    next_hour = time((start.hour+2),00)
...
def create_task(locations, all_descriptions):
    ...
    compensation = round(random.uniform(TASK_COMP[0], TASK_COMP[1]), 2) # in points
    window = random.randint(TASK_TIMEWINDOW[0], TASK_TIMEWINDOW[1])  # in minutes
    slot = random.randint(TASK_SLOT[0], TASK_SLOT[1])  # in number of people
...
def insert_tasks(db, tasks_list, start_times):
    ...
    for i, task in enumerate(tasks_list):
        ...
        query = f"INSERT INTO tasks (`location`, time_window, compensation, expired, `description`, start_time, `slot`) \
                    VALUES ('{task['location']}', {task['time_window']}, {task['compensation']}, \
                    {task['expired']}, '{task['description']}', '{start_times[i]}', {task['slot']})"
